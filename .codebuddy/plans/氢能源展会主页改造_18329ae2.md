---
name: 氢能源展会主页改造
overview: 将网站主页从APP开发介绍变更为以"氢能源展会"为主营业务的宣传网站，小方程科技作为展会主办/承办方，并在主页下方保留原APP产品（千维计算器、简码、千影）作为次要业务的独立板块。
---
...
2. Preserving APP products (千维计算器、简码、千影) as a secondary "软件产品" section
...
将现有"小方程科技"APP开发宣传网站的主页（index.html）重构为一个以**氢能源展会**为核心业务的官方网站，同时将原有的APP产品（千维计算器、简码、千影）降级为次要业务，在主页下方以独立"软件产品"板块展示。展会由**重庆博瑞特会展有限公司**主办/承办。由于暂无氢能源素材，先用占位图与占位文案搭建完整页面结构，后续可替换为真实素材。
...
7. **软件产品（次要业务）**：保留千维计算器（calc/mathcalc.html）、简码（code/scancode.html）、千影（video/linevideo.html）三个产品入口及图标，独立成板块展示。
...
5. **次要业务降级**：在主页下方设置"软件产品"板块，用卡片网格展示三个 APP（千维计算器、简码、千影），复用现有 `calc/image/icon.png`、`code/image/icon.png`、`video/image/icon.png` 图标与链接。
...
7. **软件产品（次要业务）**：独立的浅色区块，标题"软件产品"，三个等宽产品卡片（千维计算器、简码、千影），复用现有图标与链接，弱化视觉权重但保持入口清晰。
...
<item id="software-section" deps="rewrite-index">在主页新增软件产品板块，引用千维计算器、简码、千影的现有图标与链接</item>
